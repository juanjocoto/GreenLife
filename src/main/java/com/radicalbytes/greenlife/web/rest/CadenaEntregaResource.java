...
public class CadenaEntregaResource {
    ...
    public CadenaEntregaResource(CadenaEntregaRepository cadenaEntregaRepository, CadenaEntregaMapper cadenaEntregaMapper, CadenaEntregaSearchRepository cadenaEntregaSearchRepository) {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new cadenaEntregaDTO, or with status 400 (Bad Request) if the cadenaEntrega has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/cadena-entregas")
    @Timed
    public ResponseEntity<CadenaEntregaDTO> createCadenaEntrega(@Valid @RequestBody CadenaEntregaDTO cadenaEntregaDTO) throws URISyntaxException {
        log.debug("REST request to save CadenaEntrega : {}", cadenaEntregaDTO);
        if (cadenaEntregaDTO.getId() != null) {
            throw new BadRequestAlertException("A new cadenaEntrega cannot already have an ID", ENTITY_NAME, "idexists");
        }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated cadenaEntregaDTO,
     * or with status 400 (Bad Request) if the cadenaEntregaDTO is not valid,
     * or with status 500 (Internal Server Error) if the cadenaEntregaDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/cadena-entregas")
    @Timed
    public ResponseEntity<CadenaEntregaDTO> updateCadenaEntrega(@Valid @RequestBody CadenaEntregaDTO cadenaEntregaDTO) throws URISyntaxException {
     ...
     *
     * @return the ResponseEntity with status 200 (OK) and the list of cadenaEntregas in body
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the cadenaEntregaDTO, or with status 404 (Not Found)
     */
    ...
    /**
     * SEARCH  /_search/cadena-entregas?query=:query : search for the cadenaEntrega corresponding
     * to the query.
     *
    ...
    public List<CadenaEntregaDTO> searchCadenaEntregas(@RequestParam String query) {
        ...
        return StreamSupport
            .stream(cadenaEntregaSearchRepository.search(queryStringQuery(query)).spliterator(), false)
            .map(cadenaEntregaMapper::toDto)
