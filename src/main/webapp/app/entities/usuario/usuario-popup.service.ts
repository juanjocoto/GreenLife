            ...
            if (id) {
                this.usuarioService.find(id)
                    .subscribe((usuarioResponse: HttpResponse<Usuario>) => {
                        const usuario: Usuario = usuarioResponse.body;
                        if (usuario.fechaCreacion) {
                            usuario.fechaCreacion = {
                                year: usuario.fechaCreacion.getFullYear(),
                                month: usuario.fechaCreacion.getMonth() + 1,
                                day: usuario.fechaCreacion.getDate()
                            };
                        }
